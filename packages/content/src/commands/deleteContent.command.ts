...
export const deleteContentCommand = async (context: {
	...
	const entry = await (async () => {
		const fetchCorrespondingEntry = (id: string) => {
			...
			if (!entry) {
				throw new Error(`Couldn't find entry with corresponding id ${id}`);
			}
	...
	const entryDeletionHooks = (entry.hooks || [])
		.map((hook) => (hook.events.includes("delete") ? hook.onEvent : []))
		.flat();

	const globalDeletionHooks = (hooks || [])
		.map((hook) => (hook.events.includes("delete") ? hook.onEvent : []))
		.flat();
