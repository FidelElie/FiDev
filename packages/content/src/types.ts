...
export type PostCreationContext<T> = (
	context: PostContext<T>,
) => PromiseOrNot<ArrayOrNot<{ slug: string; path?: string; metadata: T }>>;

export type PostEditContext<T> = (
	context: PostContext<T>,
) => PromiseOrNot<{ path?: string; metadata: T }>;

export type PostPublishContext<T> = (context: {
	entries: PublishPostEntry<T>[];
}) => PromiseOrNot<void>;
