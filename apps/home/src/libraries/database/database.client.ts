...
export const client = {
	...
	post: {
		...
		handleRatingMusicPost: async (
			context: { slugId: string; ipAddress: string; rating: keyof typeof MusicPostRatingMap; }
		) => {
		...
		handlePostPublishing: async (entries: { slug: string; publishDate?: Date }[]) => {
